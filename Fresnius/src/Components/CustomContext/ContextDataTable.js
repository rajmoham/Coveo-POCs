          ...
          {FilteredProfileContext[0].context.map((item, index) => {
            return (
              <tr key = {item.keyName + index }>
                ...
                <td>
                  <Input autoFocus type="string" name = 'keyName' value={item.keyName} disabled={item.disabled} onChange = {(ev)=>handleInput(ev,index)}/>
                </td>
                <td>
                  <Input type="string" name = 'keyValue' value={item.keyValue} disabled={item.disabled} onChange = {(ev)=>handleInput(ev,index)}/>
                </td>
                <td>
                  <Icon style={{cursor : 'pointer', opacity : item.disabled? 0.5 : 1}} icon ={trash2} onClick={(ev)=> !item.disabled && handleDelete(ev,index)}/>
                </td>
              </tr>
            );
          })}
